...
```
stacks_cicd        <- Root directory. Both monorepo and polyrepo are supported.
│
├── stacks_cicd       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
* An automated integration test is provided (in `.github/workflows/stacks_cicd-run-tests.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
* Run `databricks configure --profile stacks_cicd-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile stacks_cicd-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)
